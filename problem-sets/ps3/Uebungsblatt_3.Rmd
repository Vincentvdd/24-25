...
deskriptives <- eu_makro |>
  ...
  summarize(Mittel = round(mean(debt_share_country, na.rm=TRUE),2),
            ...
            Max = round(max(debt_share_country, na.rm=TRUE),2)) |>  arrange(desc(Median))
...
```{r "2_h"}
final = g5 + ggtitle("Schuldenquote einzelner EU-Laender von 1995 - 2023") + geom_hline(yintercept = 0.6, color = "red")
...
```{r "3_b",optional=TRUE}
eu_2005f = ggplot(filter(eu_makro, year == 2005), aes(debt_share_country)) + geom_histogram(binwidth = 0.01) + ylab("Anzahl") + xlab("Schuldenquote") + geom_vline(xintercept = 0.6, color = "red", linetype = "dashed")
...
```{r "3_c",optional=TRUE}
eu_vergleich = ggplot(filter(eu_makro, year == 2005 | year == 2010 | year == 2015 | year == 2020), aes(debt_share_country)) + geom_histogram(binwidth = 0.01) + geom_vline(xintercept = 0.6, color = "red", linetype = "dashed") + xlab("Schuldenquote") + ylab("Anzahl") + ggtitle("Verteilung der Schuldenquote") + facet_wrap(year ~ ., nrow = 4)
...
```{r "3_e"}
balken = mutate(eu_analyse, typ = as.factor(ifelse(debt_share_country > 0.6, 1, 0)), relation = debt_share_country - 0.6)
...
```{r "3_h"}
flipped_bars = ggplot(filter(balken, year == 2005 | year == 2023), aes(x = country, y = relation)) + geom_bar(stat = "identity", aes(fill = typ), width = 0.5) + xlab("Laender") + ylab("Relation") + ggtitle("Einhaltung der Maastricht Kriterien in 2005 und 2023") + facet_wrap(year ~ ., ncol = 2) + coord_flip()
...
```{r "3_j",optional=TRUE}
animated_bars = ggplot(balken, aes(x = country, y = relation)) + geom_bar(stat = "identity", aes(fill = typ), width = 0.5) + xlab("Laender") + ylab("Relation") + ggtitle("Einhaltung der Maastricht Kriterien in 2005 und 2023") + scale_fill_manual(name = "Rel. Schuldenquote", labels = c("Erfuellt Maastricht", "Verletzt Maastricht"), values = c(`0` = "springgreen4", `1` = "red")) + coord_flip()
